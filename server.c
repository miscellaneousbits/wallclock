...
static int populate_gap_service(struct server *server)
{
    ...
    */
    ...
    /*
        ...
        callback.
    */
    if (!gatt_db_attribute_write(tmp, 0, (void *) server->device_name,
                                 server->name_len,
                                 BT_ATT_OP_WRITE_REQ,
                                 NULL, confirm_write,
                                 NULL)) {
    ...
    */
    ...
    if (!gatt_db_attribute_write(tmp, 0, (void *) &appearance,
                                 sizeof(appearance),
                                 BT_ATT_OP_WRITE_REQ,
                                 NULL, confirm_write,
                                 NULL)) {
    ...
    */
    ...
    if (!gatt_db_attribute_write(tmp, 0, (void *) preference,
                                 sizeof(preference),
                                 BT_ATT_OP_WRITE_REQ,
                                 NULL, confirm_write,
                                 NULL)) {
...
static int restart_advert(void)
{
    ...
    static uint8_t cmd1[32] = {
        0x1E, 0x02, 0x01, 0x1A, 0x1A, 0xFF, 0x4C, 0x00,
        0x02, 0x15, 0xE2, 0x0A, 0x39, 0xF4, 0x73, 0xF5,
        0x4B, 0xC4, 0xA1, 0x2F, 0x17, 0xD1, 0xAD, 0x07,
        0xA9, 0x61, 0x00, 0x00, 0x00, 0x00, 0xC8, 0x00
    };
    static uint8_t cmd2[32] = {
        0x0c, 0x0a, 0x09, 'W', 'a', 'l', 'l', 'c',
        'l', 'o', 'c', 'k', 0x00, 0x00, 0x00, 0x00,
